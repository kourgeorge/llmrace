...
import { cerebrasAdapter } from "../../src/providers/cerebras";
...
describe("cerebras-adapter", () => {
  ...
  it("calls the Cerebras endpoint with correct headers", async () => {
  ...
  it("strips cerebras/ prefix from model id", async () => {
    const fetchSpy = vi.fn().mockResolvedValue({
      ok: true,
      body: createSSEStream([
        JSON.stringify({ choices: [{ delta: { content: "Hi" } }] }),
    ...
    expect(callBody.model).toBe("llama-3.3-70b");
    globalThis.fetch = originalFetch;
  });
