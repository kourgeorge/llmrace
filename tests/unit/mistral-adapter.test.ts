...
describe("mistral-adapter", () => {
  ...
  it("calls the Mistral endpoint with correct headers", async () => {
  ...
  it("strips mistral/ prefix from model id", async () => {
    const fetchSpy = vi.fn().mockResolvedValue({
      ok: true,
      body: createSSEStream([
        JSON.stringify({ choices: [{ delta: { content: "Hi" } }] }),
    ...
    expect(callBody.model).toBe("mistral-small-latest");
    globalThis.fetch = originalFetch;
  });
